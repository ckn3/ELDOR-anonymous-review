#!/usr/bin/env python3
"""
Train semantic segmentation on GoldMDD cropped patches with MF-Mamba.
...
from review_support import portable_metadata
import os
...
def add_mfmamba_repo_to_syspath(repo: Path) -> None:
    repo = repo.resolve()
    if not repo.exists():
        raise FileNotFoundError(f"MF-Mamba repo not found: {repo}")
    repo_str = str(repo)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--data-root", type=Path, default=Path((os.environ['ELDOR_PATCH_ROOT'])))
    p.add_argument("--work-dir", type=Path, default=Path((os.environ['ELDOR_EXPERIMENT_ROOT'])))
    p.add_argument("--run-name", type=str, default=None)
    p.add_argument(
        "--mfmamba-repo",
        type=Path,
        default=Path((os.environ['ELDOR_THIRD_PARTY'] + '/MF-Mamba')),
        help="Local clone of https://github.com/Mango-Mars/MF-Mamba",
    )
    p.add_argument(
        "--hrnet-pretrained",
        type=Path,
        default=None,
        help="Optional HRNet-W18 pretrained path. If omitted and absent, backbone stays randomly initialized.",
    )
...
def build_model(mfmamba_repo: Path, hrnet_pretrained: Path | None = None) -> nn.Module:
    add_mfmamba_repo_to_syspath(mfmamba_repo)
    from MF_Mamba import MF_Mamba

    model = MF_Mamba(num_classes=NUM_FOREGROUND_CLASSES)
    if hrnet_pretrained is not None:
        hrnet_pretrained = hrnet_pretrained.expanduser().resolve()
        if not hrnet_pretrained.exists():
            raise FileNotFoundError(f"--hrnet-pretrained not found: {hrnet_pretrained}")
        # Override default random init from upstream with explicit pretrained backbone.
        model.backbone.init_weights(str(hrnet_pretrained))
        print(f"Loaded HRNet pretrained backbone: {hrnet_pretrained}", flush=True)
    return model
    ...
    torch.save(
        {
            ...
            "best_miou": best_miou,
            "args": portable_metadata(vars(args)),
        },
        path,
    )
...
def main() -> None:
    ...
    model = build_model(args.mfmamba_repo, args.hrnet_pretrained)
    model.to(device)
    ...
    run_name = args.run_name or "mfmamba_hrnet18"
    run_dir = args.work_dir / run_name
    run_dir.mkdir(parents=True, exist_ok=True)
    log_csv = run_dir / "train_log.csv"
    config_json = run_dir / "config.json"
    config_json.write_text(json.dumps(portable_metadata(vars(args)), indent=2, default=str), encoding="utf-8")
    ...
    print(f"Model: MF-Mamba + HRNet-W18 (classes={NUM_FOREGROUND_CLASSES}, bg ignored)")
