#!/usr/bin/env python3
"""
RelationNet trainer for GoldMDD MLC.
Uses ResNet50 + BCE loss.
Paper: Hua-YS/Multi-Scene-Recognition, TGRS2020
"""
import os
...
REPO     = MLC_ROOT / 'repos/RelationNet'
...
def build_relationnet_model():
    from cnn import ResNetBaseline
    resnet50 = tv.resnet50(weights=tv.ResNet50_Weights.IMAGENET1K_V1)
    model = ResNetBaseline(resnet50, NUM_CLASSES)
    return model
...
def main():
    ...
    model = build_relationnet_model().to(device)
    total     = sum(p.numel() for p in model.parameters()) / 1e6
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6
    logger.info(f"RelationNet (ResNet50): {total:.2f}M total, {trainable:.2f}M trainable")
    ...
    # RelationNet original loss: BCE (MultiLabelSoftMarginLoss)
    ...
    for epoch in range(start_ep, epochs + 1):
        ...
        logger.info(f"[relationnet] Epoch {epoch:03d}/{epochs} | "
