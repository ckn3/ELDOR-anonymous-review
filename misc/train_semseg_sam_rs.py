...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--priors-root", type=Path, default=Path((os.environ['ELDOR_ROOT'] + '/data-cropped-samrs-priors')))
    ...
    p.add_argument("--ssrs-repo", type=Path, default=Path((os.environ['ELDOR_THIRD_PARTY'] + '/SSRS/SAM_RS')))
    p.add_argument("--segment-anything-path", type=Path, default=Path((os.environ['ELDOR_THIRD_PARTY'] + '/sam-hq')))
    p.add_argument("--sam-checkpoint", type=Path, default=Path((os.environ['ELDOR_PRETRAINED'] + '/sam_vit_h_4b8939.pth')))
...
def main() -> None:
    ...
    (run_dir / "config.json").write_text(json.dumps(portable_metadata(vars(args)), indent=2, default=str), encoding="utf-8")
