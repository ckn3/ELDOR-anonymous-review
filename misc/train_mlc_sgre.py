...
with a simple classification head (SGRE's full MLIC model requires
word embeddings which are dataset-specific).
"""
import os
...
REPO     = MLC_ROOT / 'repos/SGRE'

# SGRE path must be first to use its models/
...
class SGREModel(nn.Module):
    """SGRE ResNet-101 backbone + classification head."""
    def __init__(self, num_classes: int):
        super().__init__()
        # Use SGRE's registered resnet101 backbone
        from models.factory import backbone_entrypoint
        create_fn, feat_dim = backbone_entrypoint('resnet101')
        backbone = create_fn(True)  # pretrained=True
        self.backbone   = backbone
        self.pool       = nn.AdaptiveAvgPool2d(1)
        self.classifier = nn.Linear(feat_dim, num_classes)

    def forward(self, x):
        feat = self.backbone(x)  # [B, seq, 2048] or [B, 2048, H, W]
        if feat.dim() == 3:
            feat = feat.mean(dim=1)   # [B, 2048] — sequence mean pooling
        elif feat.dim() == 4:
...
def main():
    ...
    model = SGREModel(num_classes=NUM_CLASSES).to(device)
    total     = sum(p.numel() for p in model.parameters()) / 1e6
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6
    logger.info(f"SGRE (ResNet-101): {total:.2f}M total, {trainable:.2f}M trainable")
    ...
    # SGRE original loss: ASL from their own lib
    sys.path.insert(0, str(REPO / 'lib'))
    from aslloss import AsymmetricLoss as SGRE_ASL
    criterion = SGRE_ASL(gamma_neg=4, gamma_pos=0, clip=0.05)
    ...
    for epoch in range(start_ep, epochs + 1):
        ...
        logger.info(f"[sgre] Epoch {epoch:03d}/{epochs} | "
