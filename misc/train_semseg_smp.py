#!/usr/bin/env python3
"""
Train semantic segmentation on GoldMDD cropped patches with SMP+timm encoders.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--arch", type=str, default="deeplabv3plus", choices=["deeplabv3plus", "fpn", "unet"])
    p.add_argument("--encoder", type=str, default="convnext_tiny", help="timm encoder name without 'tu-' prefix")
    p.add_argument("--encoder-weights", type=str, default="imagenet", help="SMP encoder weights, e.g. imagenet or none")
...
def build_model(arch: str, encoder: str, encoder_weights: str | None) -> nn.Module:
    enc_name = f"tu-{encoder}"
    enc_weights = None if (encoder_weights is None or str(encoder_weights).lower() == "none") else encoder_weights
...
def main() -> None:
    ...
    run_name = args.run_name or f"{args.arch}_{args.encoder}"
    ...
    print(f"Model: {args.arch} + tu-{args.encoder} (classes={NUM_FOREGROUND_CLASSES}, bg ignored)")
