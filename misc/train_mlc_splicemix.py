#!/usr/bin/env python3
"""SpliceMix trainer for GoldMDD MLC — stripped of DDP."""
import os
...
REPO     = MLC_ROOT / 'repos/SpliceMix'
sys.path.insert(0, str(REPO))  # MUST be first to avoid models/ conflict
...
# SpliceMix ResNet-101 — REPO must be in sys.path for loss_fns
from models.ResNet_101 import model as SpliceMixModel
...
def main():
    ...
    # SpliceMix ResNet-101 — pretrained ImageNet
    model = SpliceMixModel(
    ...
    logger.info(f"SpliceMix (ResNet-101): {total:.2f}M total, {trainable:.2f}M trainable")
    ...
    # SpliceMix original loss: BCELoss (MultiLabelSoftMarginLoss)
    import torch.nn as nn2
    criterion = nn2.MultiLabelSoftMarginLoss()
    ...
    for epoch in range(start_ep, epochs + 1):
        ...
        logger.info(f"[splicemix] Epoch {epoch:03d}/{epochs} | "
