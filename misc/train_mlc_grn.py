...
Paper: jiankang1991/GRN-SNDL, TGRS2021
Note: Using BCE only (skip NCA metric loss — requires memory bank).
"""
import os
...
def build_grn_model():
    from model import ResNet18_cls
    return ResNet18_cls(clsNum=NUM_CLASSES, dim=128)
...
def main():
    ...
    model = build_grn_model().to(device)
    total     = sum(p.numel() for p in model.parameters()) / 1e6
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6
    logger.info(f"GRN (ResNet18): {total:.2f}M total, {trainable:.2f}M trainable")
    ...
    # GRN BCE loss (classification part)
    ...
    # GRN forward returns (embedding, logits)
    def grn_fwd(m, x):
        _, logits = m(x)
        return logits
    ...
    for epoch in range(start_ep, epochs + 1):
        ...
        val_metrics = evaluate(model, val_loader, device, forward_fn=grn_fwd)
        elapsed = time.time() - t0
        logger.info(f"[grn] Epoch {epoch:03d}/{epochs} | "
    ...
    model.load_state_dict(ck['model'])
    test_metrics = evaluate(model, test_loader, device, forward_fn=grn_fwd)
