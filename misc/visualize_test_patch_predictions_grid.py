...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--experiments-root", type=Path, default=Path((os.environ['ELDOR_EXPERIMENT_ROOT'])))
    ...
    p.add_argument(
        "--output",
        type=Path,
        default=Path((os.environ['ELDOR_EXPERIMENT_ROOT'] + '/diagnostics/test_random15_methods_grid.png')),
    )
    p.add_argument(
        "--sample-list-output",
        type=Path,
        default=Path((os.environ['ELDOR_EXPERIMENT_ROOT'] + '/diagnostics/test_random15_methods_grid_samples.txt')),
    )
    return p.parse_args()
...
def build_efficientvit_model(cfg: dict) -> tuple[nn.Module, object]:
    misc_dir = Path(cfg.get("work_dir", (os.environ['ELDOR_EXPERIMENT_ROOT']))).parent / "misc"
    # Fallback to canonical misc path if work_dir doesn't map cleanly.
    if not misc_dir.exists():
        misc_dir = Path((os.environ['ELDOR_ROOT'] + '/misc'))
