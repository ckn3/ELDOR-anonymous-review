#!/usr/bin/env python3
"""
Train semantic segmentation on GoldMDD cropped patches with SegFormer (HuggingFace Transformers).
...
try:
    ...
    raise SystemExit(
        "transformers is required for SegFormer training. Install with: "
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="HuggingFace SegFormer checkpoint. Head size will be remapped to 14 classes.",
    )
    p.add_argument(
        "--pretrained",
        action="store_true",
...
def build_model(model_name: str, pretrained: bool) -> nn.Module:
    if pretrained:
        ...
        # Reinitialize segmentation head if user disables pretrained.
...
def main() -> None:
    ...
    model = build_model(args.model_name, pretrained=args.pretrained)
    model.to(device)
    ...
    print(f"Model: SegFormer ({args.model_name}) (classes={NUM_FOREGROUND_CLASSES}, bg ignored)")
