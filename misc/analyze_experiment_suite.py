...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--experiments-root",
        type=Path,
        default=Path((os.environ['ELDOR_EXPERIMENT_ROOT'])),
    )
    p.add_argument(
        "--output-dir",
        type=Path,
        default=Path((os.environ['ELDOR_EXPERIMENT_ROOT'] + '/diagnostics')),
    )
    return p.parse_args()
...
def main() -> None:
    ...
    plot_per_class_iou_heatmap(
        ...
        split_dist_csv=Path((os.environ['ELDOR_ROOT'] + '/train_val_test_class_distribution_merged.csv')),
    )
    write_summary_md(
        summary_df,
        runs,
