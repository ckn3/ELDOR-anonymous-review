...
GOLDMDD = Path(os.environ["ELDOR_ROOT"])
ROOT = GOLDMDD.parent
SRC_ROOT = Path(os.environ["ELDOR_DATA_ROOT"])
OUT_ROOT = Path(os.environ["ELDOR_PATCH_ROOT"])
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--overwrite", action="store_true", help="Replace an existing cropped output directory.")
    return p.parse_args()
...
def prepare_output_root(out_root: Path, overwrite: bool = False) -> None:
    resolved = out_root.resolve()
    source = SRC_ROOT.resolve()
    project = GOLDMDD.resolve()
    if (resolved == source or resolved in source.parents or source in resolved.parents
            or resolved == project or resolved in project.parents):
        raise ValueError("Crop output must be separate from the source data and cannot contain the project.")
    if out_root.exists():
        if not overwrite:
            raise FileExistsError(f"Crop output already exists: {out_root}. Use a new path or --overwrite.")
...
def build_crops(workers: int, overwrite: bool = False) -> tuple[list[SiteCropStats], dict[str, int]]:
    prepare_output_root(OUT_ROOT, overwrite=overwrite)
...
def main() -> None:
    args = parse_args()
    site_stats, patch_counts = build_crops(workers=args.workers, overwrite=args.overwrite)
