...
def build_smp(arch: str, encoder: str, encoder_weights: str = "imagenet"):
    mod = load_module_from_path("train_semseg_smp_mod", Path((os.environ['ELDOR_ROOT'] + '/misc/train_semseg_smp.py')))
...
def build_segformer():
    mod = load_module_from_path("train_semseg_segformer_mod", Path((os.environ['ELDOR_ROOT'] + '/misc/train_semseg_segformer.py')))
...
def build_efficientvit():
    mod = load_module_from_path("train_semseg_evit_mod", Path((os.environ['ELDOR_ROOT'] + '/misc/train_semseg_efficientvit.py')))
    repo = Path((os.environ['ELDOR_THIRD_PARTY'] + '/efficientvit'))
...
def build_farseg():
    mod = load_module_from_path("train_semseg_farseg_mod", Path((os.environ['ELDOR_ROOT'] + '/misc/train_semseg_farseg.py')))
    ns = argparse.Namespace(
        model="farseg",
        farseg_repo=Path((os.environ['ELDOR_THIRD_PARTY'] + '/FarSeg')),
        pretrained=True,
...
def build_rsseg_logcan():
    repo = Path((os.environ['ELDOR_THIRD_PARTY'] + '/rssegmentation'))
...
def build_pem():
    os.environ.setdefault("GOLDMDD_PEM_ROOT", (os.environ['ELDOR_ROOT'] + '/data-cropped-pem'))
    os.environ.setdefault("GOLDMDD_DATA_CROPPED_ROOT", (os.environ['ELDOR_PATCH_ROOT']))
    return build_detectron2_family(
        Path((os.environ['ELDOR_THIRD_PARTY'] + '/PEM')),
        Path((os.environ['ELDOR_THIRD_PARTY'] + '/PEM/configs/goldmdd/semantic-segmentation/pem_R50_bs8_658k.yaml')),
        "pem_train_mod",
        "PEM",
    )
...
def build_mask2former():
    ...
    os.environ.setdefault("GOLDMDD_DATA_CROPPED_ROOT", (os.environ['ELDOR_PATCH_ROOT']))
    return build_detectron2_family(
        Path((os.environ['ELDOR_THIRD_PARTY'] + '/Mask2Former')),
        Path((os.environ['ELDOR_THIRD_PARTY'] + '/Mask2Former/configs/goldmdd/semantic-segmentation/mask2former_R50_bs8_658k.yaml')),
        "m2f_train_mod",
        "Mask2Former-R50",
    )
...
def build_ssaseg(config_path: Path, family_name: str):
    ...
    repo = Path((os.environ['ELDOR_THIRD_PARTY'] + '/SSA-Seg'))
    sys.path.insert(0, str(repo))
    try:
...
def build_geoseg(config_path: Path, family_name: str):
    repo = Path((os.environ['ELDOR_THIRD_PARTY'] + '/GeoSeg'))
    sys.path.insert(0, str(repo))
    try:
...
def build_ppmambaseg():
    repo = Path((os.environ['ELDOR_THIRD_PARTY'] + '/PPMambaSeg/GeoSeg'))
    sys.path.insert(0, str(repo))
    try:
        from tools.cfg import py2cfg  # type: ignore
...
def build_rs3mamba():
    mod = load_module_from_path("train_semseg_rs3mamba_mod", Path((os.environ['ELDOR_ROOT'] + '/misc/train_semseg_rs3mamba.py')))
    model = mod.build_model(
        Path((os.environ['ELDOR_THIRD_PARTY'] + '/SSRS/RS3Mamba')),
        Path((os.environ['ELDOR_THIRD_PARTY'] + '/SSRS/RS3Mamba/pretrain/vmamba_tiny_e292.pth')),
    )
...
def build_mfmamba():
    mod = load_module_from_path("train_semseg_mfmamba_mod", Path((os.environ['ELDOR_ROOT'] + '/misc/train_semseg_mfmamba.py')))
    model = mod.build_model(
        Path((os.environ['ELDOR_THIRD_PARTY'] + '/MF-Mamba')),
        None,
    )
...
def build_mcpnet():
    repo = Path((os.environ['ELDOR_THIRD_PARTY'] + '/MCPNet'))
    sys.path.insert(0, str(repo))
    try:
        ...
        cfg = Config.fromfile(
            str(
                Path(
                    (os.environ['ELDOR_THIRD_PARTY'] + '/MCPNet/configs/goldmdd_mcpnet_full_80ep_bs8_ce.py')
                )
            )
        )
...
def build_sam_rs(model_name: str):
    mod = load_module_from_path("train_semseg_samrs_mod", Path((os.environ['ELDOR_ROOT'] + '/misc/train_semseg_sam_rs.py')))
    model = mod.build_model(Path((os.environ['ELDOR_THIRD_PARTY'] + '/SSRS/SAM_RS')), model_name)
...
def build_sam2_1_hierabplus():
    mod = load_module_from_path(
        ...
        Path((os.environ['ELDOR_ROOT'] + '/misc/train_semseg_sam_family.py')),
    )
    ns = argparse.Namespace(
        family="sam2_1",
        decoder_type="fpn_multiscale",
        decoder_channels=256,
        sam2_root=Path((os.environ['ELDOR_THIRD_PARTY'] + '/sam2')),
        sam2_config="configs/sam2.1/sam2.1_hiera_b+.yaml",
        sam2_checkpoint=None,
        sam2_image_size=512,
        sam3_root=Path((os.environ['ELDOR_THIRD_PARTY'] + '/sam3')),
        sam3_checkpoint=None,
        sam3_load_from_hf=False,
        sam3_image_size=1008,
        hqsam_root=Path((os.environ['ELDOR_THIRD_PARTY'] + '/sam-hq')),
...
def build_rsamseg_vit_b():
    mod = load_module_from_path(
        ...
        Path((os.environ['ELDOR_ROOT'] + '/misc/train_semseg_rsamseg.py')),
    )
    encoder_cfg = mod._build_encoder_cfg("vit_b", img_size=512)
    model = mod.RSAMSegSemantic(
        Path((os.environ['ELDOR_THIRD_PARTY'] + '/RSAM-Seg')),
        encoder_cfg,
        num_classes=14,
    )
...
def main():
    ...
    builders = {
        ...
        "ssaseg_afformer_base": lambda: build_ssaseg(
            Path((os.environ['ELDOR_THIRD_PARTY'] + '/SSA-Seg/configs/goldmdd/afformer_base_goldmdd.py')),
            "Afformer-Base",
        ),
        "ssaseg_seaformer_base": lambda: build_ssaseg(
            Path((os.environ['ELDOR_THIRD_PARTY'] + '/SSA-Seg/configs/goldmdd/seaformer_base_goldmdd.py')),
            "SeaFormer-Base",
        ),
        "ssaseg_segnext_tiny": lambda: build_ssaseg(
            Path((os.environ['ELDOR_THIRD_PARTY'] + '/SSA-Seg/configs/goldmdd/segnext_tiny_goldmdd.py')),
            "SegNeXt-Tiny",
        ),
        "ssaseg_cgrseg_b": lambda: build_ssaseg(
            Path((os.environ['ELDOR_THIRD_PARTY'] + '/SSA-Seg/configs/goldmdd/cgrseg_b_goldmdd.py')),
            "CGRSeg-B",
        ),
        "ssaseg_upernet_swin_tiny": lambda: build_ssaseg(
            Path((os.environ['ELDOR_THIRD_PARTY'] + '/SSA-Seg/configs/goldmdd/upernet_swin_tiny_goldmdd.py')),
            "UPerNet-Swin-Tiny",
        ),
        "ssaseg_ocrnet_hr48": lambda: build_ssaseg(
            Path((os.environ['ELDOR_THIRD_PARTY'] + '/SSA-Seg/configs/goldmdd/ocrnet_hr48_goldmdd.py')),
            "OCRNet-HR48",
        ),
        "geoseg_unetformer": lambda: build_geoseg(
            Path((os.environ['ELDOR_THIRD_PARTY'] + '/GeoSeg/config/goldmdd/unetformer.py')),
            "UNetFormer-R18",
        ),
        "geoseg_a2fpn": lambda: build_geoseg(
            Path((os.environ['ELDOR_THIRD_PARTY'] + '/GeoSeg/config/goldmdd/a2fpn.py')),
            "A2FPN-R18",
        ),
        "geoseg_abcnet": lambda: build_geoseg(
            Path((os.environ['ELDOR_THIRD_PARTY'] + '/GeoSeg/config/goldmdd/abcnet.py')),
            "ABCNet-R18",
        ),
        "geoseg_banet": lambda: build_geoseg(
            Path((os.environ['ELDOR_THIRD_PARTY'] + '/GeoSeg/config/goldmdd/banet.py')),
            "BANet-ResT-Lite",
        ),
        "geoseg_manet": lambda: build_geoseg(
            Path((os.environ['ELDOR_THIRD_PARTY'] + '/GeoSeg/config/goldmdd/manet.py')),
            "MANet-R50",
        ),
        "geoseg_dcswin": lambda: build_geoseg(
            Path((os.environ['ELDOR_THIRD_PARTY'] + '/GeoSeg/config/goldmdd/dcswin.py')),
            "DC-Swin-Small",
        ),
        "geoseg_pyramidmamba": lambda: build_geoseg(
            Path((os.environ['ELDOR_THIRD_PARTY'] + '/GeoSeg/config/goldmdd/pyramidmamba.py')),
