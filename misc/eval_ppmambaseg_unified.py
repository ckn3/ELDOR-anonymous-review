...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--repo-root",
        type=Path,
        default=Path((os.environ['ELDOR_THIRD_PARTY'] + '/PPMambaSeg/GeoSeg')),
    )
    p.add_argument(
        "--config-path",
        type=Path,
        default=Path((os.environ['ELDOR_THIRD_PARTY'] + '/PPMambaSeg/GeoSeg/config/goldmdd/ppmamba.py')),
    )
    p.add_argument(
        "--run-dir",
        type=Path,
        default=Path((os.environ['ELDOR_EXPERIMENT_ROOT'] + '/ppmambaseg_ppmamba_goldmdd_b8_e80_poly_ce_dice')),
    )
