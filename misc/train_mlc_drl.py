...
Uses ResampleLoss with rebalancing for long-tail class distribution.
"""
import os
...
class DRLModel(nn.Module):
    """ResNet-101 + classification head for DRL."""
...
def build_drl_loss():
    """Build ResampleLoss (DRL standalone) with GoldMDD class frequencies."""
    sys.path.insert(0, str(REPO))
    from resample_loss_standalone import ResampleLoss

    freq_path  = str(MLC_ROOT / 'protocols/goldmdd_class_freq.pkl')
    split_path = str(MLC_ROOT / 'protocols/goldmdd_class_split.pkl')
...
def main():
    ...
    model = DRLModel(num_classes=NUM_CLASSES).to(device)
    total     = sum(p.numel() for p in model.parameters()) / 1e6
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6
    logger.info(f"DRL (ResNet-101): {total:.2f}M total, {trainable:.2f}M trainable")
    ...
    for epoch in range(start_ep, epochs + 1):
        ...
        logger.info(f"[drl] Epoch {epoch:03d}/{epochs} | "
