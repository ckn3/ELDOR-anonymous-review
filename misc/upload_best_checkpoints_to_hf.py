...
def main():
    ...
    ap.add_argument("--repo-id", required=True, help="Destination model repository owned by the review account")
    ap.add_argument("--repo-type", default="model")
    ap.add_argument("--root", default=(os.environ['ELDOR_EXPERIMENT_ROOT']))
    ap.add_argument(
        "--manifest",
        default=(os.environ['ELDOR_EXPERIMENT_ROOT'] + '/diagnostics/hf_best_checkpoints_manifest.csv'),
    )
    ...
    extras = [
        (
            (os.environ['ELDOR_EXPERIMENT_ROOT'] + '/diagnostics/summary.md'),
