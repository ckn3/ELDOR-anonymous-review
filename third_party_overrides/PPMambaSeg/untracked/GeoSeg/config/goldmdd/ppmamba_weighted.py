...
from geoseg.losses import DiceLoss, JointLoss
...
weights_name = "ppmambaseg_ppmamba_goldmdd_b8_e80_poly_weighted_ce_dice"
...
# Keep default backbone/pretraining for consistency with base PPMamba run.
net = PPMamba(num_classes=num_classes, pretrained=True)
loss = JointLoss(
    nn.CrossEntropyLoss(weight=torch.tensor(CLASS_WEIGHTS, dtype=torch.float32), ignore_index=ignore_index),
